import Image from 'next/image';
import styles from './KeyFeatures.module.css';

export default function KeyFeatures() {
  return (
      <section className={styles.keyFeatures}>
        <div className={styles.r1}>
          <div className={styles.r2}>
            <div className={styles.dektop}>
              <div className={styles.tag} data-border="true">
                <div className={styles.icon}>
                  <div className={styles.r6}>
                    <Image className={styles.r7} src="/assets/media/CATPIZzB7Cpdua0pawm4Rflc0.svg" alt="Icon" width={21} height={20} />
                  </div>
                </div>
                <div className={styles.text}>
                  <p className={styles.r9}>
                    Key Features
                  </p>
                </div>
              </div>
              <div className={styles.textContainer}>
                <div className={styles.heading}>
                  <h2 className={styles.r12}>
                    Why Sellers Choose Us
                  </h2>
                </div>
                <div className={styles.paragraph}>
                  <p className={styles.r14}>
                    Built around one simple idea: payments should start where your customers already are. Every feature is made to get you paid faster and more securely.
                  </p>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className={styles.subContainer}>
          <div className={styles.container}>
            <div className={styles.r1}>
              <div className={styles.r17}>
                <div className={styles.tablet} data-border="true">
                  <div className={styles.iconContainer} data-border="true">
                    <div className={styles.icon2}>
                      <div className={styles.r21}>
                        <Image className={styles.r22} src="/assets/media/CBmOgAnexNM95avuCV5yAqNY.svg" alt="Icon" width={24} height={24} />
                      </div>
                    </div>
                  </div>
                  <div className={styles.textContainer2}>
                    <div className={styles.heading2}>
                      <h3 className={styles.r25}>
                        Turnkey Checkout
                      </h3>
                    </div>
                    <div className={styles.paragraph2}>
                      <p className={styles.r27}>
                        Your store and checkout are ready in about 30 minutes, no developer required.
                      </p>
                    </div>
                  </div>
                  <div className={styles.image}>
                    <div className={styles.r29}>
                      <Image className={styles.r30} src="/assets/media/kNNjNmCjmCdk1p9PDGxhoU28Y0.png" alt="" width={229} height={275} aria-hidden="true" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.subContainer2}>
              <div className={styles.r1}>
                <div className={styles.r32}>
                  <div className={styles.noImage} data-border="true">
                    <div className={styles.iconContainer} data-border="true">
                      <div className={styles.icon2}>
                        <div className={styles.r21}>
                          <Image className={styles.r22} src="/assets/media/EtUt4dE9Qq1CfTNHrv5GlJRIR4.svg" alt="Icon" width={24} height={24} />
                        </div>
                      </div>
                    </div>
                    <div className={styles.textContainer3}>
                      <div className={styles.heading3}>
                        <h3 className={styles.r36}>
                          Reliable Support
                        </h3>
                      </div>
                      <div className={styles.paragraph3}>
                        <p className={styles.r38}>
                          Onboarding help and support are available any time you need a hand.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.r1}>
                <div className={styles.r32}>
                  <div className={styles.noImage} data-border="true">
                    <div className={styles.iconContainer} data-border="true">
                      <div className={styles.icon2}>
                        <div className={styles.r21}>
                          <Image className={styles.r22} src="/assets/media/1za2KZAbXQ3Vdi3TPz3PO0P6IUg.svg" alt="Icon" width={24} height={24} />
                        </div>
                      </div>
                    </div>
                    <div className={styles.textContainer3}>
                      <div className={styles.heading3}>
                        <h3 className={styles.r36}>
                          Secure by Design
                        </h3>
                      </div>
                      <div className={styles.paragraph3}>
                        <p className={styles.r38}>
                          PCI-compliant hosted payments keep every transaction safe.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles.container2}>
            <div className={styles.subContainer3}>
              <div className={styles.r1}>
                <div className={styles.r41}>
                  <div className={styles.noImage2} data-border="true">
                    <div className={styles.iconContainer} data-border="true">
                      <div className={styles.icon2}>
                        <div className={styles.r21}>
                          <Image className={styles.r22} src="/assets/media/zgsf9HuO6ahvWmH7TiSlo603eU.svg" alt="Icon" width={24} height={24} />
                        </div>
                      </div>
                    </div>
                    <div className={styles.textContainer4}>
                      <div className={styles.heading3}>
                        <h3 className={styles.r36}>
                          Grows With You
                        </h3>
                      </div>
                      <div className={styles.paragraph4}>
                        <p className={styles.r45}>
                          Handle one order or thousands - the same simple checkout scales with your sales.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className={styles.r1}>
                <div className={styles.r46}>
                  <div className={styles.noImage3} data-border="true">
                    <div className={styles.iconContainer} data-border="true">
                      <div className={styles.icon2}>
                        <div className={styles.r21}>
                          <Image className={styles.r22} src="/assets/media/oUvN49Ctu2IChXmom9uSHgXwUE.svg" alt="Icon" width={24} height={24} />
                        </div>
                      </div>
                    </div>
                    <div className={styles.textContainer3}>
                      <div className={styles.heading3}>
                        <h3 className={styles.r36}>
                          Merchant-focused
                        </h3>
                      </div>
                      <div className={styles.paragraph3}>
                        <p className={styles.r38}>
                          Features built around real seller workflows, not generic agency extras.
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.r1}>
              <div className={styles.r48}>
                <div className={styles.tablet2} data-border="true">
                  <div className={styles.iconContainer} data-border="true">
                    <div className={styles.icon2}>
                      <div className={styles.r21}>
                        <Image className={styles.r22} src="/assets/media/9S6VY8NoE6wgrzZwVwwEFY8ukQ.svg" alt="Icon" width={24} height={24} />
                      </div>
                    </div>
                  </div>
                  <div className={styles.textContainer5}>
                    <div className={styles.heading2}>
                      <h3 className={styles.r25}>
                        Data You Can Trust
                      </h3>
                    </div>
                    <div className={styles.paragraph5}>
                      <p className={styles.r52}>
                        Every order and settlement appears in real time, so numbers never surprise you.
                      </p>
                    </div>
                  </div>
                  <div className={styles.image2}>
                    <div className={styles.r54}>
                      <Image className={styles.r55} src="/assets/media/kNNjNmCjmCdk1p9PDGxhoU28Y0.png" alt="" width={229} height={275} aria-hidden="true" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
  );
}
